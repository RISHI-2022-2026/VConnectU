...
export const RoadmapView: React.FC<{ skillName?: string }> = ({ skillName }) => {
  const [skillInput, setSkillInput] = useState(skillName || '');
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-6 hover:border-slate-300 dark:hover:border-slate-600 transition">
                ...
                <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-lg border border-slate-200 dark:border-slate-800/50">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        {item.resources.map((res: any, i) => {
                            const isPlaceholder = res.url?.includes('example.com') || !res.url;
                            const searchUrl = `https://www.youtube.com/results?search_query=${encodeURIComponent(res.searchQuery || res.title + ' ' + skillInput)}`;
                            const finalUrl = isPlaceholder ? searchUrl : res.url;
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex-1">
                                        <div className="font-medium line-clamp-1">{res.title}</div>
                                        <div className="text-xs text-slate-500 dark:text-slate-400 capitalize">
                                            {res.type} {isPlaceholder && '• Search'}
                                        </div>
                                    </div>
