...
export const SkillList: React.FC<SkillListProps> = ({ user, onUpdateUser }) => {
  ...
  const fetchSuggestions = async () => {
    ...
    try {
        ...
        const result = await apiService.suggestSkills(known, goals);
        const filtered = result.skills.filter((s: string) => !known.includes(s) && !goals.includes(s));
  ...
  const updateUserAndDB = async (updatedUser: User) => {
      ...
      await apiService.updateSkills(updatedUser.skillsKnown, updatedUser.skillsToLearn);
  };
  ...
  const handleCompleteQuiz = (score: number) => {
    ...
    apiService.updateSkills(updatedUser.skillsKnown, updatedUser.skillsToLearn);
    onUpdateUser(updatedUser);
    setQuizSkill(null);
  };
