...
import { SkillList } from "@/components/skillvouch/SkillList";
...
export default function SkillVouchPage() {
    ...
    return (
        <div className="animate-in fade-in space-y-8">
            ...
            <div className="flex items-center gap-4 border-b border-slate-200 pb-6">
                ...
                <div>
                    <h1 className="text-3xl font-bold text-slate-900">Skill Verification & Exchange</h1>
                    <p className="text-slate-500 mt-1">Verify your expertise and find peers to learn from.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-8">
                    <SkillList 
                        user={userProfile} 
                        onUpdateUser={(u) => setUserProfile(u as User)} 
                    />
                </div>

                {/* Right Column - Features */}
                <div className="space-y-8">
                    <MatchFinder currentUser={userProfile} onMessageUser={(id) => alert(`Messaging peer ${id} is not fully integrated yet!`)} />
                    {userProfile.skillsToLearn.length > 0 && (
                        <RoadmapView skillName={userProfile.skillsToLearn[0]} />
                    )}
                </div>
            </div>
        </div>
